...
In the diagram, these values are $A=100$, $\alpha = 0.4$, and $\beta = 0.6$.
...
This suggests that the elasticity of substitution for a Cobb-Douglas production function falls between $[0,\infty )$.
The exact \textbf{elasticity of substitution is $1$.}
\par


\paragraph{Constant Elasticity of Substitution Production Function (CES)}

Each of the three special production functions are cases of a function called the \textbf{constant elasticity of substitution (CES) production function}, given by the equation:
\[Q = [aL^{\frac{\sigma - 1}{\sigma}} + bK^{\frac{\sigma -1}{\sigma}}]^{\frac{\sigma}{\sigma - 1}}\]
where $a$, $b$, and $\sigma$ are positive constants ($\sigma$ is the elasticity of substitution).
As $\sigma$ varies between $0$ and $\infty$, the shape of the isoquants go in order of
\begin{enumerate}
    \item fixed production function (L-shape)
    \item Cobb-Douglas production function (curve)
    \item linear production function (straight line)
\end{enumerate}

\begin{center}
    \begin{tabular}{||p{0.3\linewidth} p{0.36\linewidth} p{0.3\linewidth}||}
        \hline
        Production Function & Elasticity of Substitution ($\sigma$) & Other Characteristics\\ [0.5ex]
        \hline\hline
        Linear production function & $\sigma = \infty$ & Inputs are perfect substitutes.\\
        \hline
        CES production function & $0\le \sigma \le \infty$ & Other three prod. func. are subsets.\\
        \hline
        Fixed-proportions production function & $\sigma = 0$ & Inputs are perfect complements.\\ [1ex]
        \hline
    \end{tabular}
\end{center}
...
When a firm scales it's inputs, it's outputs must also scale.
\par

\newtheorem*{returnScale}{returns to scale}
\begin{returnScale}
    The concept that output of a firm will increase by a certain percentage when the input increases by a percentage.
\end{returnScale}

Returns to scale is defined as:
\[Returns\ to\ scale = \frac{\%\Delta (quantity\ of\ output)}{\%\Delta (quantity\ of\ input)}\]
\par

A particular firm uses labor $L$ and capital $K$ to produce output $Q$.
Suppose \textbf{all inputs are scaled by the same proportionate amount $\lambda$}, where $\lambda > 1$. If 
\[L \longrightarrow \lambda L\]
then
\[K \longrightarrow \lambda K\]
Let $\phi$ represent the proportionate increase in the quantity of output $Q$.

\begin{itemize}
    \item If $\phi > \lambda$, it is \textbf{increasing returns to scale.} Some increase in input will result in a disproportionate increase in output.

    \item If $\phi = \lambda$, it is \textbf{constant returns to scale.} Some increase in input will result in an equal increase in output.

    \item If $\phi < \lambda$, it is \textbf{decreasing returns to scale.} Some increase in input will result in a smalle increase in output.
\end{itemize}

\begin{figure}[tb]
\centering 
\includegraphics[width=0.95\columnwidth]{returnToScale} 
\caption[]{Increasing, Constant, and Decreasing Returns to Scale}
\label{fig:returnToScale}
\end{figure}


%-----------------------------
\subsection{Returns to Scale versus Diminishing Marginal Returns}
\textbf{Returns to scale} pertains to the impact of an increase in \textbf{all inputs at once} (full derivative), while \textbf{marginal returns} (marginal product $MP$) pertains to the impact of \textbf{increasing a single input} (partial derivative).


\begin{figure}[tb]
\centering 
\includegraphics[width=0.95\columnwidth]{marginalReturns} 
\caption[]{Marginal Returns versus Returns to Scale}
\label{fig:marginalReturns}
\end{figure}

In figure ~\ref{fig:marginalReturns} it can be seen that if a single input is doubled, the marginal return (output) does not scale proportionally. 
\par

In contrast, doubling all inputs results in the output doubling. This is an example of the returns to scale.

\newtheorem*{techProg}{technological progress}
\begin{techProg}
    A change in production that enables a firm to achieve more output (efficiency).
\end{techProg}

\newtheorem*{neutralTech}{neutral technological progress}
\begin{neutralTech}
    A firm can produce more with the same inputs, but the isoquant curve does not change, therefore the $MRTS_{L,K}$ is the same.
\end{neutralTech}

\newtheorem{laborTech}{labor-saving technological progress}
\begin{laborTech}
    Progress that causes $MP_K$ to increase relative to $MP_L$.
    This shifts the isoquant inwards.
\end{laborTech}

\newtheorem{capitalTech}{capital-saving technological progress}
\begin{capitalTech}
    Progress that causes $MP_L$ to increase relative to $MP_K$.
    This shifts the isoquant inwards.
\end{capitalTech}
...
%	Chapter 7
%--------------------------------------------------
\section{Costs and Cost Minimization}

Identify and apply different concepts of costs that factor into a firm's decision making, including explicit vs implicit costs, opportunity costs, economic vs accounting costs, and sunk vs nonsunk costs.


%--------------------------------
\subsection{Cost Concepts for Decision Making}

To economists, cost is the value of sacrificed opportunities (opportunity cost).

\newtheorem*{expCost}{explicit costs}
\begin{expCost}
    Costs that involve a direct monetary outlay (spent money).
\end{expCost}

\newtheorem*{impCost}{implicit costs}
\begin{impCost}
    Costs that do not involve outlays of cash (opportunity cost).
\end{impCost}

\newtheorem*{econCosts}{economic costs}
\begin{econCosts}
    The sum of a firm's explicit and implicit costs.
\end{econCosts}

\newtheorem*{accCosts}{accounting costs}
\begin{accCosts}
    The total of a firm's explicit costs that have been incurred in the past.
\end{accCosts}

\newtheorem*{sunkCosts}{sunk costs}
\begin{sunkCosts}
    Costs that have already been incurred, and cannot be recovered.
\end{sunkCosts}

\newtheorem*{nonsunkCosts}{nonsunk costs}
\begin{nonsunkCosts}
    Costs that are only incurred on a particular decision.
\end{nonsunkCosts}

If a firm were deciding on whether they should build a factory, at the moment the factory would be a \textit{nonsunk cost}. On purchase, the factory becomes a \textit{sunk cost}.


%--------------------------
\subsection{The Cost-Minimization Problem}

\newtheorem*{costMin}{cost-minimization problem}
\begin{costMin}
    Problem of finding the input combination that minimizes a firm's total cost of producing a certain level of output.
\end{costMin}

In the long run a firm may vary all input levels, in contrast a firm in the short run is not able to change at least one of their inputs of production.
\end{costMin}


\paragraph{Long-Run Cost-Minimization Problem}

The cost-minimization problem is an example of constrained optimization.
\par

The price of one unit of labor is called \textbf{wage} $w$, and the price per unit of \textbf{capital services} is $r$.
Hiring employees is an explicit cost, whereas if the owner of a firm provides his own labor, this would be an implicit cost (because they are not paid a wage, and sacrifice their time at a job).
The price of capital could be implicit if they were leasing out their own assets (they can't provide services to other firms), and an explicit cost if they leased capital services from another company (buying computer time for example on offsite servers).
\par

Firms must choose a quantity of capital and labor that minimize the total cost $TC$ of production for a particular output $Q$.
\[TC = wL + rK\]


\paragraph{Isocost Lines}

\begin{figure}[tb]
\centering 
\includegraphics[width=0.95\columnwidth]{isocost} 
\caption[]{Isocost Lines}
\label{fig:isocost}
\end{figure}

Isocost lines are used to solve the cost-minimization problem graphically.
An isocost line represents all the combinations of $K$ and $L$ that result in the same $TC$.
\par

For example, let $w = 10$ per labor-hour, $r = 20$ per machine-hour, and $TC = \$1MM$.
The \$1M isocost line is described by the equation
\[1000000 = 10L + 20K\]
\[K = \frac{1000000}{20} - \frac{10L}{20}\]
\par

The slope of an isocost line is $\frac{-w}{r}$. The K-axis intercept (vertical axis) is $\frac{TC}{r}$.


\paragraph{Graphical Characterization of the Solution to the Long-Run Cost-Minimization Problem}

\begin{figure}[tb]
\centering 
\includegraphics[width=0.95\columnwidth]{costMinInput} 
\caption[]{Cost-Minimizing Input Combination}
\label{fig:costMinInput}
\end{figure}

Point a of figure ~\ref{fig:costMinInput} is the solution to the cost-min problem.
Points not on the isoquant are inefficient. Point on the isoquant, but not tangent to the isocost line are efficient, but not cost minimizing for a particular output level.
\par

\textbf{Cost-minimizing solution occurs when:}
\begin{equation*}
    \begin{aligned}
...
In figure ~\ref{fig:costMinInput}, input combination $A$ is an \textbf{interior optimum} (which requires that all inputs are $>0$).
The previous equation could also be rewritten in ratio form:
\[\frac{MP_L}{w} = \frac{MP_K}{r}\]
This condition shows that at the local optimization, each additional output per dollar spent on labor services equals the additional output per dollar spent on capital services.
Thus, when costs are minimized, we get an equal "bang for the buck" from each input.
\par

Consider point $E$.
The slope of the isoquant is more negative than the slope of the isocost line.
\begin{equation*}
    \begin{aligned}
        -\frac{MP_L}{MP_K} &< -\frac{w}{r}\\
        \frac{MP_L}{w} &> -\frac{MP_K}{r}\\
    \end{aligned}
\end{equation*}
This shows that every dollar spent on labor will save more than a dollar in capital services.


\paragraph{Corner Point Solutions}

\begin{figure}[tb]
\centering 
\includegraphics[width=0.95\columnwidth]{cornerPointMin} 
\caption[]{Corner Point Solution to the Cost-Minimization Problem}
\label{fig:cornerPointMin}
\end{figure}

These are optimal solutions in which there is no tangency between the isoquant and the isocost curve.
This would happen in a situation where one of the two inputs is not needed at all; every dollar spent on labor yields more output than every dollar spent on capital (for example, figure ~\ref{fig:cornerPointMin}).
\[\frac{MP_L}{w} > \frac{MP_K}{r}\]
It would be in the firm's best interest to substitute all capital for only labor.
\par


%-----------------------------------------------
\subsection{Comparative Statics Analysis of the Cost-Minimization Problem}


\paragraph{Analysis of Changes in Input Prices (Comparative Statics)}

When the price of one input ($w$ or $r$) increases (or decreases), holding everything else constant, it causes the firm to substitute one input for another.
\par

When an input increases in price, it causes the slope of the isocost to change, as well as the intercepts.
The tangency point between the isocost line and isoquant changes as a result.
\par
This relationship relies on two assumptions:
\begin{enumerate}
    \item With a smooth isoquant that has a diminishing $MRTS_{L,K}$, and positive quantities of an input, \textbf{an increase in the price of that input will cause the quantity of that input to go down} (to the new cost-minimizing value).
    \item When the input is initially zero-quantity, or the firm has a fixed proportions prod-func, \textbf{an increase in the price of that input will not affect the quantity of that input}.
\end{enumerate}


\paragraph{Analysis of Changes in Output (Comparative Statics)}

\newtheorem{expPath*}{expansion path}
\begin{expPath}
    A line that connects the cost-minimizing input combinations as the quantity of output changes.
\end{expPath}

The cost-minimizing point on isoquants does not necessarily follow a linear path.
As the output level changes, the cost-minimizing combinations of inputs follows along the \textbf{expansion path}.
\par
\begin{itemize}
    \item If an input increases as the output increases, this input is a \textbf{normal input}.
    \item If an input decreases as output increases, this input is an \textbf{inferior input}.
\end{itemize}
Both inputs can be normal, but only one of the two inputs may be inferior.
An input may be inferior if upon expanding production a firm automates the process resulting in less input being required.
\par


\paragraph{Summary of Input Demand Comparative Statics Analysis}

The \textbf{labor demand curve} describes how a change in the price $w$ of labor will affect the units of labor demanded. A change in the price of labor causes a \textbf{movement along the labor demand curve}. A change in output level causes a \textbf{shift in the labor demand curve}.
\par

If an input is normal, and output increases, demand for that input will increase. If the input is inferior, the demand curve for that input will shift left.
\par

The firm's \textbf{capital demand curve} is the exact same.


\paragraph{Price Elasticity of Demand for Inputs}

The sensitivity of the demand for a product to it's price (change in) is referred to as it's elasticity.
The \textbf{price elasticity of demand for labor}, $\epsilon_{L,w}$, is the \% change in the cost-minimizing quantity of labor for a 1\% change in the price of labor ($w$).
\begin{equation*}
    \begin{aligned}
        \epsilon_{L,w} &= \frac{\frac{\Delta L}{L} \times 100\%}{\frac{\Delta w}{w} \times 100\%}\\
                       &= \frac{\Delta L}{\Delta w} \frac{w}{L}\\
    \end{aligned}
\end{equation*}
The \textbf{price elasticity of demand for capital}, $\epsilon_{K,r}$ is the exact same.
\[\epsilon_{K,w} &= \frac{\Delta L}{\Delta w} \frac{w}{L}\]
\par

A determinant of the price elasticity for inputs is the \textit{elasticity of substitution}.
When the price elasticity of demand for labor is small, the demand for labor is relatively insensitive to the price of labor.
